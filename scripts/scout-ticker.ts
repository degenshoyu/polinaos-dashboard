...
async function main() {
  const { tickers, topN, outDir, raw, mode, csvPath, debug, showVariants } =
    parseArgs();
  if (!tickers.length) {
    console.error(
      'Usage: ts-node -P tsconfig.scripts.json -r tsconfig-paths/register scripts/scout-ticker.ts "BONK WIF ..." ' +
        "[--mode both|cands|resolve] [--top 20] [--out .tmp] [--csv ./file.csv] [--raw] [--debug] [--variants]",
    );
    process.exit(1);
  }
