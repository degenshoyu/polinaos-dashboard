// components/summary/SearchSummaryCard.tsx
"use client";
      ...
      <div className="relative p-5 md:p-6">
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-1 text-3xl font-extrabold text-white">{compact(all.tweets)}</div>
            <ul className="mt-3 space-y-1.5">
              <MetricInline label="Impressions" value={compact(all.views)} />
              <MetricInline label="Engagements" value={compact(all.engagements)} />
              <MetricInline label="ER" value={pct(all.er)} highlight />
            </ul>
          </div>

          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
            <div className="text-xs uppercase tracking-wide text-emerald-300 font-semibold">Verified Tweets</div>
            <div className="mt-1 text-3xl font-extrabold text-white">{compact(ver.tweets)}</div>
            <ul className="mt-3 space-y-1.5">
              <MetricInline label="Impressions" value={compact(ver.views)} />
              <MetricInline label="Engagements" value={compact(ver.engagements)} />
        ...
        <div className="mt-6">
          ...
          <ol className="space-y-2">
            {(topShillers || []).slice(0, 3).map((s, i) => (
              <li key={`${s.handle}-${i}`} className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2">
            ...
            {(!topShillers || topShillers.length === 0) && <li className="text-sm text-gray-400">No shillers found</li>}
          </ol>
        </div>
      </div>
...
function TrophyBadge({ rank }: { rank: 1 | 2 | 3 }) {
  const scheme =
    rank === 1
      ? { grad: "from-amber-300 to-yellow-500", glow: "shadow-[0_0_26px_rgba(250,204,21,0.35)]", chip: "bg-amber-300 text-[#0a0f0e]" }
      : rank === 2
      ? { grad: "from-slate-200 to-slate-400", glow: "shadow-[0_0_24px_rgba(148,163,184,0.3)]", chip: "bg-slate-200 text-[#0a0f0e]" }
      : { grad: "from-orange-300 to-amber-500", glow: "shadow-[0_0_24px_rgba(251,146,60,0.3)]", chip: "bg-orange-300 text-[#0a0f0e]" };
  ...
  return (
    <li
      className={`flex items-center gap-2 rounded-md border px-2 py-1.5
        ${highlight ? "border-emerald-400/30 bg-emerald-400/10 shadow-[0_0_18px_rgba(16,185,129,0.15)]" : "border-white/10 bg-white/[0.03]"}`}
    >
      <span className={`${highlight ? "text-emerald-200 font-semibold" : "text-gray-400"}`}>{label}</span>
      <span className={`${highlight ? "text-white font-bold" : "text-white/90 font-semibold"}`}>{value}</span>
    </li>
  );
}
