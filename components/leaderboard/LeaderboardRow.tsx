...
export function LeaderboardRow({
   ...
   *  - Fallback to normalized tokenDisplay when tokenKey is missing
   *  - Sum counts for the same key
   */
  const coinsForUI = React.useMemo(() => {
    type CoinItem = { tokenKey: string; tokenDisplay: string; count: number };
    const merged = new Map<string, CoinItem>();

    for (const c of coinsAll || []) {
      const keyRaw =
        (c.tokenKey && c.tokenKey.trim()) ||
        (c.tokenDisplay && c.tokenDisplay.trim()) ||
        "";
      if (!keyRaw) continue;

      // Normalize key to ensure stable uniqueness across renders
      ...
      if (prev) {
        ...
        merged.set(key, {
          tokenKey: key, // stable, unique key used by CoinsChipList item rendering
    ...
    // Optional: sort by count desc then name asc for nicer, deterministic UI
  ...
  return (
    <div className="grid grid-cols-12 gap-2 px-3 py-3 items-center">
      ...
      {/* Coins — use merged, stable-key list */}
      <div className="col-span-2 flex items-center justify-end">
        <CoinsChipList coins={coinsForUI} max={6} />
      </div>
    </div>
  );
}
